...
function SearchResultCard() {
  const { authors, contents, datetime, publisher, thumbnail, title, translators } = mockSearchResult.documents[0];
    ...
    <div className="searchResultCard">
      <div className="contentSec">
       ...
       <div className="contents">
         <h6 className="space-2x"><strong>{title}</strong></h6>
         <p className="space-2x">{authors} • {translators.length >= 1 && `옮긴이 ${translators}`} • {publisher} • {moment(datetime).format('YYYY.MM.DD')} </p>
         <p className="descriptionSec">
           {contents}
         </p>
       </div>
      </div>
      <div className="buttonSec">
        <button className="Btn-default Btn-sm">등록 시작</button>
      </div>
    </div>
  )
}
